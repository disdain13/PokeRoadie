...
namespace PokeRoadie.Extensions
{
    public static class PokeRoadieExtensions
    {
        ...
        public static string GetMinStats(this PokemonData pokemon)
        {
            ...
            {
                return $"{(String.IsNullOrWhiteSpace(pokemon.DeployedFortId) ? "" : "^") + (pokemon.Favorite == 1 ? "*" : "")}{name.PadRight(19)} " + $"({pokemon.CalculatePokemonValue()}V-{pokemon.Cp.ToString()}CP-{pokemon.GetPerfection().ToString("0.00")}IV-LV{pokemon.GetLevel().ToString("00")}-{pokemon.StaminaMax.ToString()}HP)".PadRight(33);
            }
            else
            {
