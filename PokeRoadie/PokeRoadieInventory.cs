...
namespace PokeRoadie
{
    public class PokeRoadieInventory
    {
        ...
        //    var pokemonList = myPokemon.Where(p => p.DeployedFortId == 0 && p.Favorite == 0 && p.Cp < PokeRoadieSettings.Current.KeepAboveCP).ToList();
        //    if (PokeRoadieSettings.Current.PokemonsNotToTransfer != null)
        //        pokemonList = pokemonList.Where(p => !PokeRoadieSettings.Current.PokemonsNotToTransfer.Contains(p.PokemonId)).ToList();
        ...
        //            if (PokeRoadieSettings.Current.PrioritizeIVOverCP)
        //            {
